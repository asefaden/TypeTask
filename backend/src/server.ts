...
app.post('/api/auth/login', async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (rows.length === 0) return res.status(400).json({ error: 'ኢሜይል ወይም ይለፍ ቃል የተሳሳተ ነው' });

        const user = rows[0];
        const isPasswordValid = await bcrypt.compare(password, user.password);
        if (!isPasswordValid) return res.status(400).json({ error: 'ኢሜይል ወይም ይለፍ ቃል የተሳሳተ ነው' });
...
app.post('/api/tasks', authenticateToken, async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        res.json({ id: (result as any).insertId, title, description, status: 'pending' });
    } catch (error: any) {
        res.status(500).json({ error: error.message });
    }
});
...
app.get('/api/debug/users', async (req: Request, res: Response) => {
    try {
        const [users]: any = await pool.query('SELECT id, username, email, created_at FROM users');
